...
class pyWordArt:
    def __init__(self, text="WordArt Test", style=15, size=100, noOpenGL=False):
        ...
        self.Styles = {'outline': 0, 'up': 1, 'arc': 2, 'squeeze': 3, 'inverted-arc': 4, 'basic-stack': 5,
                       'italic-outline': 6, 'slate': 7, 'mauve': 8, 'graydient': 9, 'red-blue': 10, 'brown-stack': 11,
                       'radial': 12, 'purple': 13, 'green-marble': 14, 'rainbow': 15, 'aqua': 16, 'texture-stack': 17,
                       'paper-bag': 18, 'sunset': 19, 'tilt': 20, 'blues': 21, 'yellow-dash': 22, 'green-stack': 23,
                       'chrome': 24, 'marble-slab': 25, 'gray-block': 26, 'superhero': 27, 'horizon': 28,
                       'stack-3d': 29}
        ...
        self.canvasWidth = 800  # 3508
        self.canvasHeight = 600  # 2480
        self.debug = False
    ...
    def toFile(self, filename):
        ...
        if not bool(self.imgName.endswith(".png") or self.imgName.endswith(".jpg") or self.imgName.endswith(
                ".jpeg") or self.imgName.endswith(".gif") or self.imgName.endswith(".tif") or self.imgName.endswith(
                ".tiff") or self.imgName.endswith(".bmp")):
